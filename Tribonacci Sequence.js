...
function tribonacci(signature,n){
  let myArray = [];
  let sigVarOne = signature[0];
  let sigVarTwo = signature[1];
  let sigVarThree = signature[2];
  let newValue = 0;
  if (n === 0){
    return myArray;
  } else if (n === 1){
    myArray.push(sigVarOne);
    return myArray;
  } else if (n === 2){
    myArray.push(sigVarOne, sigVarTwo);
    return myArray;
  } else {
    myArray.push(sigVarOne, sigVarTwo, sigVarThree);
    for(let i=3;i<n;i++){
      newValue = myArray[i-3] + myArray[i-2] + myArray[i-1];
